...
fun ExportData(
    ...
    val accessories by exportDataViewModel.allAccessories.collectAsState(
        emptyList()
    )
                    ...
                    stickyHeader {
                        Text(
                            modifier = Modifier
                                ...
                                .background(MaterialTheme.colorScheme.primaryContainer)
                                .padding(8.dp),
                            text = stringResource(R.string.hdr_accessories),
                            style = MaterialTheme.typography.bodyMedium,
                        )
                    }
                    items(items = accessories, key = { "accessory-${it.uid}" }) {
                        Text(text = it.name)
                    }
